...
static void test_execution_table_is_validation_subset(void) {
    ...
    /* 178 prior mappings + 11 residual rounding/bitwise/average mappings. */
    assert(count == 189u);
...
static void test_representative_semantics(void) {
    ...
    assert(descriptor->result_shape == TURBOWASM_V128_RAW);
    ...
    /* Still validated but intentionally deferred to #53/#54. */
